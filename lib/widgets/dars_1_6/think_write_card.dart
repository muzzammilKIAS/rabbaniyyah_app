...
class ThinkWriteCard6 extends StatefulWidget {
  const ThinkWriteCard6({super.key});

  @override
  State<ThinkWriteCard6> createState() => _ThinkWriteCard6State();
}

class _ThinkWriteCard6State extends State<ThinkWriteCard6> {
  ...
  final List<GlobalKey<WordOrderActivityRowState>> _woKeys =
      List.generate(Dars116.wordOrder.length, (_) => GlobalKey<WordOrderActivityRowState>());
  ...
  void initState() {
    ...
    final rawMatch = app.dars116Get<Map<String, dynamic>>('matchPairs', {});
    _matchInitial = rawMatch.map((k, v) => MapEntry(int.parse(k), v as int));

    final savedFill = app.dars116Get<Map<String, dynamic>>('fillAnswers', {});
    _fillAnswers = List.generate(Dars116.fillItems.length, (i) => savedFill[i.toString()] as String?);

    _woInitial = List.generate(Dars116.wordOrder.length, (i) {
      final saved = app.dars116Get<String>('wo$i', '');
  ...
  void _checkFill() {
    setState(() {
      _fillResults = List.generate(Dars116.fillItems.length, (i) {
        if (_fillAnswers[i] == null) return null;
        return _fillAnswers[i] == Dars116.fillItems[i].answer;
      });
    });
  }
    ...
    return SectionCard(
      child: Column(
        ...
        children: [
          ...
          MatchPairsActivity(
            key: _matchKey,
            rightItems: Dars116.matchWords,
            leftItems: Dars116.matchSentences,
            correctIndexForRight: Dars116.matchWordCorrectIndex,
            rightHeader: 'الْكَلِمَةُ',
            leftHeader: 'الْجُمْلَةُ',
            initialPairs: _matchInitial,
            onChanged: (pairs) => context.read<AppState>().dars116Set('matchPairs', pairs.map((k, v) => MapEntry(k.toString(), v))),
          ),
          Row(
            children: [
              ElevatedButton.icon(
              ...
              ScoreBadge(
                ...
                total: _matchKey.currentState?.results != null ? Dars116.matchWords.length : null,
              ),
            ],
          ),
          ...
          const SizedBox(height: 14),
          for (var i = 0; i < Dars116.fillItems.length; i++)
            Container(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  Text(Dars116.fillItems[i].before, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: Dars116.fillBank.map((w) => DropdownMenuItem(value: w, child: Text(w))).toList(),
                        onChanged: (v) {
                          setState(() {
                            _fillAnswers[i] = v;
                            _fillResults = null;
                          });
                          context.read<AppState>().dars116Set('fillAnswers', {for (var j = 0; j < _fillAnswers.length; j++) j.toString(): _fillAnswers[j]});
                        },
                      ),
                    ),
                  ),
                  Text(Dars116.fillItems[i].after, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
          ...
          Row(
            children: [
              ...
              ScoreBadge(correct: fillCorrectCount, total: fillCorrectCount != null ? Dars116.fillItems.length : null),
            ],
          ),
          const SizedBox(height: 24),
          Divider(color: c.border),
          ...
          for (var i = 0; i < Dars116.wordOrder.length; i++) ...[
            WordOrderActivityRow(
              key: _woKeys[i],
              index: i,
              item: Dars116.wordOrder[i],
              initialSelected: _woInitial[i],
              onChanged: (sel) => context.read<AppState>().dars116Set('wo$i', sel.join('|')),
            ),
            if (i != Dars116.wordOrder.length - 1) const SizedBox(height: 18),
          ],
        ],
      ),
    );
