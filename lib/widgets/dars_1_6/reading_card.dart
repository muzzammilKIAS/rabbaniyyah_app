...
class ReadingCard6 extends StatefulWidget {
  const ReadingCard6({super.key});

  @override
  State<ReadingCard6> createState() => _ReadingCard6State();
}

class _ReadingCard6State extends State<ReadingCard6> {
  int? _speakingLine;
  ...
  Future<void> _speakLine(int i) async {
    ...
    await tts.speak(Dars116.readingLines[i]);
  ...
  Future<void> _playAll() async {
    ...
    for (var i = 0; i < Dars116.readingLines.length; i++) {
      if (!mounted || _cancelAll) return;
      setState(() => _speakingLine = i);
      await app.tts.speak(Dars116.readingLines[i]);
    }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('(${Dars116.hadithSource})', style: TextStyle(fontSize: 12, color: c.textMuted)),
              ],
            ),
          ),
        ],
