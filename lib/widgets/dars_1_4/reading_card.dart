...
class ReadingCard4 extends StatefulWidget {
  const ReadingCard4({super.key});

  @override
  State<ReadingCard4> createState() => _ReadingCard4State();
}

class _ReadingCard4State extends State<ReadingCard4> {
  int? _speakingLine;
  ...
  Future<void> _speakLine(int i) async {
    ...
    await tts.speak(Dars114.readingLines[i]);
  ...
  Future<void> _playAll() async {
    ...
    for (var i = 0; i < Dars114.readingLines.length; i++) {
      if (!mounted || _cancelAll) return;
      setState(() => _speakingLine = i);
      await app.tts.speak(Dars114.readingLines[i]);
    }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('(${Dars114.hadithSource})', style: TextStyle(fontSize: 12, color: c.textMuted)),
              ],
            ),
          ),
        ],
