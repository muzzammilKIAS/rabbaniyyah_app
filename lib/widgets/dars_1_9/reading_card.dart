...
class ReadingCard9 extends StatefulWidget {
  const ReadingCard9({super.key});

  @override
  State<ReadingCard9> createState() => _ReadingCard9State();
}

class _ReadingCard9State extends State<ReadingCard9> {
  int? _speakingLine;
  ...
  Future<void> _speakLine(int i) async {
    ...
    await tts.speak(Dars119.readingLines[i]);
  ...
  Future<void> _playAll() async {
    ...
    for (var i = 0; i < Dars119.readingLines.length; i++) {
      if (!mounted || _cancelAll) return;
      setState(() => _speakingLine = i);
      await app.tts.speak(Dars119.readingLines[i]);
    }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('(${Dars119.hadithSource})', style: TextStyle(fontSize: 12, color: c.textMuted)),
              ],
            ),
          ),
        ],
