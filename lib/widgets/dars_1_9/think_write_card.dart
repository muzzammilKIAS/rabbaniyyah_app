...
class ThinkWriteCard9 extends StatefulWidget {
  const ThinkWriteCard9({super.key});

  @override
  State<ThinkWriteCard9> createState() => _ThinkWriteCard9State();
}

class _ThinkWriteCard9State extends State<ThinkWriteCard9> {
  ...
  final List<GlobalKey<WordOrderActivityRowState>> _woKeys =
      List.generate(Dars119.wordOrder.length, (_) => GlobalKey<WordOrderActivityRowState>());
  ...
  void initState() {
    ...
    final rawMatch = app.dars119Get<Map<String, dynamic>>('matchPairs', {});
    _matchInitial = rawMatch.map((k, v) => MapEntry(int.parse(k), v as int));

    final savedFill = app.dars119Get<Map<String, dynamic>>('fillAnswers', {});
    _fillAnswers = List.generate(Dars119.fillItems.length, (i) => savedFill[i.toString()] as String?);

    _woInitial = List.generate(Dars119.wordOrder.length, (i) {
      final saved = app.dars119Get<String>('wo$i', '');
  ...
  void _checkFill() {
    setState(() {
      _fillResults = List.generate(Dars119.fillItems.length, (i) {
        if (_fillAnswers[i] == null) return null;
        return _fillAnswers[i] == Dars119.fillItems[i].answer;
      });
    });
  }
    ...
    return SectionCard(
      child: Column(
        ...
        children: [
          ...
          MatchPairsActivity(
            key: _matchKey,
            rightItems: Dars119.matchWords,
            leftItems: Dars119.matchSentences,
            correctIndexForRight: Dars119.matchWordCorrectIndex,
            rightHeader: 'الْكَلِمَةُ',
            leftHeader: 'الْجُمْلَةُ',
            initialPairs: _matchInitial,
            onChanged: (pairs) => context.read<AppState>().dars119Set('matchPairs', pairs.map((k, v) => MapEntry(k.toString(), v))),
          ),
          Row(
            children: [
              ElevatedButton.icon(
              ...
              ScoreBadge(
                ...
                total: _matchKey.currentState?.results != null ? Dars119.matchWords.length : null,
              ),
            ],
          ),
          ...
          const SizedBox(height: 14),
          for (var i = 0; i < Dars119.fillItems.length; i++)
            Container(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  Text(Dars119.fillItems[i].before, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: Dars119.fillBank.map((w) => DropdownMenuItem(value: w, child: Text(w))).toList(),
                        onChanged: (v) {
                          setState(() {
                            _fillAnswers[i] = v;
                            _fillResults = null;
                          });
                          context.read<AppState>().dars119Set('fillAnswers', {for (var j = 0; j < _fillAnswers.length; j++) j.toString(): _fillAnswers[j]});
                        },
                      ),
                    ),
                  ),
                  Text(Dars119.fillItems[i].after, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
          ...
          Row(
            children: [
              ...
              ScoreBadge(correct: fillCorrectCount, total: fillCorrectCount != null ? Dars119.fillItems.length : null),
            ],
          ),
          const SizedBox(height: 24),
          Divider(color: c.border),
          ...
          for (var i = 0; i < Dars119.wordOrder.length; i++) ...[
            WordOrderActivityRow(
              key: _woKeys[i],
              index: i,
              item: Dars119.wordOrder[i],
              initialSelected: _woInitial[i],
              onChanged: (sel) => context.read<AppState>().dars119Set('wo$i', sel.join('|')),
            ),
            if (i != Dars119.wordOrder.length - 1) const SizedBox(height: 18),
          ],
        ],
      ),
    );
