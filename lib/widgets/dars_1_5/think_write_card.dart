...
class ThinkWriteCard5 extends StatefulWidget {
  const ThinkWriteCard5({super.key});

  @override
  State<ThinkWriteCard5> createState() => _ThinkWriteCard5State();
}

class _ThinkWriteCard5State extends State<ThinkWriteCard5> {
  ...
  final List<GlobalKey<WordOrderActivityRowState>> _woKeys =
      List.generate(Dars115.wordOrder.length, (_) => GlobalKey<WordOrderActivityRowState>());
  ...
  void initState() {
    ...
    final rawMatch = app.dars115Get<Map<String, dynamic>>('matchPairs', {});
    _matchInitial = rawMatch.map((k, v) => MapEntry(int.parse(k), v as int));

    final savedFill = app.dars115Get<Map<String, dynamic>>('fillAnswers', {});
    _fillAnswers = List.generate(Dars115.fillItems.length, (i) => savedFill[i.toString()] as String?);

    _woInitial = List.generate(Dars115.wordOrder.length, (i) {
      final saved = app.dars115Get<String>('wo$i', '');
  ...
  void _checkFill() {
    setState(() {
      _fillResults = List.generate(Dars115.fillItems.length, (i) {
        if (_fillAnswers[i] == null) return null;
        return _fillAnswers[i] == Dars115.fillItems[i].answer;
      });
    });
  }
    ...
    return SectionCard(
      child: Column(
        ...
        children: [
          ...
          MatchPairsActivity(
            key: _matchKey,
            rightItems: Dars115.matchWords,
            leftItems: Dars115.matchSentences,
            correctIndexForRight: Dars115.matchWordCorrectIndex,
            rightHeader: 'الْكَلِمَةُ',
            leftHeader: 'الْجُمْلَةُ',
            initialPairs: _matchInitial,
            onChanged: (pairs) => context.read<AppState>().dars115Set('matchPairs', pairs.map((k, v) => MapEntry(k.toString(), v))),
          ),
          Row(
            children: [
              ElevatedButton.icon(
              ...
              ScoreBadge(
                ...
                total: _matchKey.currentState?.results != null ? Dars115.matchWords.length : null,
              ),
            ],
          ),
          ...
          const SizedBox(height: 14),
          for (var i = 0; i < Dars115.fillItems.length; i++)
            Container(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  Text(Dars115.fillItems[i].before, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: Dars115.fillBank.map((w) => DropdownMenuItem(value: w, child: Text(w))).toList(),
                        onChanged: (v) {
                          setState(() {
                            _fillAnswers[i] = v;
                            _fillResults = null;
                          });
                          context.read<AppState>().dars115Set('fillAnswers', {for (var j = 0; j < _fillAnswers.length; j++) j.toString(): _fillAnswers[j]});
                        },
                      ),
                    ),
                  ),
                  Text(Dars115.fillItems[i].after, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
          ...
          Row(
            children: [
              ...
              ScoreBadge(correct: fillCorrectCount, total: fillCorrectCount != null ? Dars115.fillItems.length : null),
            ],
          ),
          const SizedBox(height: 24),
          Divider(color: c.border),
          ...
          for (var i = 0; i < Dars115.wordOrder.length; i++) ...[
            WordOrderActivityRow(
              key: _woKeys[i],
              index: i,
              item: Dars115.wordOrder[i],
              initialSelected: _woInitial[i],
              onChanged: (sel) => context.read<AppState>().dars115Set('wo$i', sel.join('|')),
            ),
            if (i != Dars115.wordOrder.length - 1) const SizedBox(height: 18),
          ],
        ],
      ),
    );
