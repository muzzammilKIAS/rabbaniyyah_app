...
class ThinkWriteCard3 extends StatefulWidget {
  const ThinkWriteCard3({super.key});

  @override
  State<ThinkWriteCard3> createState() => _ThinkWriteCard3State();
}

class _ThinkWriteCard3State extends State<ThinkWriteCard3> {
  ...
  final List<GlobalKey<WordOrderActivityRowState>> _woKeys =
      List.generate(Dars113.wordOrder.length, (_) => GlobalKey<WordOrderActivityRowState>());
  ...
  void initState() {
    ...
    final rawMatch = app.dars113Get<Map<String, dynamic>>('matchPairs', {});
    _matchInitial = rawMatch.map((k, v) => MapEntry(int.parse(k), v as int));

    final savedFill = app.dars113Get<Map<String, dynamic>>('fillAnswers', {});
    _fillAnswers = List.generate(Dars113.fillItems.length, (i) => savedFill[i.toString()] as String?);

    _woInitial = List.generate(Dars113.wordOrder.length, (i) {
      final saved = app.dars113Get<String>('wo$i', '');
  ...
  void _checkFill() {
    setState(() {
      _fillResults = List.generate(Dars113.fillItems.length, (i) {
        if (_fillAnswers[i] == null) return null;
        return _fillAnswers[i] == Dars113.fillItems[i].answer;
      });
    });
  }
    ...
    return SectionCard(
      child: Column(
        ...
        children: [
          ...
          MatchPairsActivity(
            key: _matchKey,
            rightItems: Dars113.matchWords,
            leftItems: Dars113.matchSentences,
            correctIndexForRight: Dars113.matchWordCorrectIndex,
            rightHeader: 'الْكَلِمَةُ',
            leftHeader: 'الْجُمْلَةُ',
            initialPairs: _matchInitial,
            onChanged: (pairs) => context.read<AppState>().dars113Set('matchPairs', pairs.map((k, v) => MapEntry(k.toString(), v))),
          ),
          Row(
            children: [
              ElevatedButton.icon(
              ...
              ScoreBadge(
                ...
                total: _matchKey.currentState?.results != null ? Dars113.matchWords.length : null,
              ),
            ],
          ),
          ...
          const SizedBox(height: 14),
          for (var i = 0; i < Dars113.fillItems.length; i++)
            Container(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  Text(Dars113.fillItems[i].before, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: Dars113.fillBank.map((w) => DropdownMenuItem(value: w, child: Text(w))).toList(),
                        onChanged: (v) {
                          setState(() {
                            _fillAnswers[i] = v;
                            _fillResults = null;
                          });
                          context.read<AppState>().dars113Set('fillAnswers', {for (var j = 0; j < _fillAnswers.length; j++) j.toString(): _fillAnswers[j]});
                        },
                      ),
                    ),
                  ),
                  Text(Dars113.fillItems[i].after, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 18, color: c.text)),
          ...
          Row(
            children: [
              ...
              ScoreBadge(correct: fillCorrectCount, total: fillCorrectCount != null ? Dars113.fillItems.length : null),
            ],
          ),
          const SizedBox(height: 24),
          Divider(color: c.border),
          ...
          for (var i = 0; i < Dars113.wordOrder.length; i++) ...[
            WordOrderActivityRow(
              key: _woKeys[i],
              index: i,
              item: Dars113.wordOrder[i],
              initialSelected: _woInitial[i],
              onChanged: (sel) => context.read<AppState>().dars113Set('wo$i', sel.join('|')),
            ),
            if (i != Dars113.wordOrder.length - 1) const SizedBox(height: 18),
          ],
        ],
      ),
    );
