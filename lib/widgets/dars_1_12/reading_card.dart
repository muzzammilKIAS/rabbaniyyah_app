...
class ReadingCard12 extends StatefulWidget {
  const ReadingCard12({super.key});

  @override
  State<ReadingCard12> createState() => _ReadingCard12State();
}

class _ReadingCard12State extends State<ReadingCard12> {
  int? _speakingLine;
  ...
  Future<void> _speakLine(int i) async {
    if (!TtsService.canSpeak(Dars1112.readingLines[i])) return;
    ...
    await tts.speak(Dars1112.readingLines[i]);
  ...
  Future<void> _playAll() async {
    ...
    for (var i = 0; i < Dars1112.readingLines.length; i++) {
      if (!mounted || _cancelAll) return;
      if (!TtsService.canSpeak(Dars1112.readingLines[i])) continue;
      setState(() => _speakingLine = i);
      await app.tts.speak(Dars1112.readingLines[i]);
    }
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  if (!TtsService.canSpeak(Dars1112.readingLines[i]))
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('(${Dars1112.hadithSource})', style: TextStyle(fontSize: 12, color: c.textMuted)),
              ],
            ),
          ),
        ],
