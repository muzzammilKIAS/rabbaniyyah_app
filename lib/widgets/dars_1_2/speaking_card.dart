...
class SpeakingCard2 extends StatefulWidget {
  const SpeakingCard2({super.key});

  @override
  State<SpeakingCard2> createState() => _SpeakingCard2State();
}

class _SpeakingCard2State extends State<SpeakingCard2> {
  ...
  void initState() {
    super.initState();
    final raw = context.read<AppState>().dars112Get<Map<String, dynamic>>('speakingPairs', {});
    ...
    return SectionCard(
      child: Column(
        ...
        children: [
          ...
          MatchPairsActivity(
            key: _matchKey,
            rightItems: Dars112.matchQuestions,
            leftItems: List.generate(Dars112.matchAnswers.length, (i) => '${Dars112.matchAnswerLetters[i]}) ${Dars112.matchAnswers[i]}'),
            correctIndexForRight: Dars112.matchCorrectIndex,
            rightHeader: 'الأَسْئِلَةُ',
            leftHeader: 'الأَجْوِبَةُ',
            initialPairs: _initialPairs,
            onChanged: (pairs) => context.read<AppState>().dars112Set('speakingPairs', pairs.map((k, v) => MapEntry(k.toString(), v))),
          ),
          ...
          Row(
            children: [
              ...
              ScoreBadge(
                ...
                total: _matchKey.currentState?.results != null ? Dars112.matchQuestions.length : null,
              ),
            ],
          ),
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                for (var i = 0; i < Dars112.dialogue.length; i++)
                  Container(
                    ...
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(child: Text(Dars112.dialogue[i].studentB, textAlign: TextAlign.center, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 15.5, height: 1.7, color: c.text))),
                        Container(width: 1, height: 40, color: c.border),
                        Expanded(child: Text(Dars112.dialogue[i].studentA, textAlign: TextAlign.center, style: TextStyle(fontFamily: AppTheme.arabicFont, fontSize: 15.5, height: 1.7, fontWeight: FontWeight.w600, color: c.accent))),
                      ],
                    ),
                  ),
              ],
