...
class ReadingCard2 extends StatefulWidget {
  const ReadingCard2({super.key});

  @override
  State<ReadingCard2> createState() => _ReadingCard2State();
}

class _ReadingCard2State extends State<ReadingCard2> {
  ...
  Future<void> _speakLine(int i) async {
    ...
    await tts.speak(Dars112.readingLines[i]);
  ...
  Future<void> _playAyah() async {
    ...
    final ok = await quran.playAyah(surah: Dars112.ayahSurahNum, ayah: Dars112.ayahNum);
  ...
  Future<void> _playAll() async {
    ...
    if (!mounted || _cancelAll) return;
    setState(() => _speakingLine = -1);
    await app.quranAudio.playAyah(surah: Dars112.ayahSurahNum, ayah: Dars112.ayahNum);
