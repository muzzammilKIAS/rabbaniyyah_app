...
class ReadingCard11 extends StatefulWidget {
  const ReadingCard11({super.key});

  @override
  State<ReadingCard11> createState() => _ReadingCard11State();
}

class _ReadingCard11State extends State<ReadingCard11> {
  ...
  Future<void> _speakLine(int i) async {
    if (!TtsService.canSpeak(Dars1111.readingLines[i])) return;
    ...
    await tts.speak(Dars1111.readingLines[i]);
  ...
  Future<void> _playAyah() async {
    ...
    final ok = await quran.playAyah(surah: Dars1111.ayahSurahNum, ayah: Dars1111.ayahNum);
  ...
  Future<void> _playAll() async {
    ...
    for (var i = 0; i < Dars1111.readingLines.length; i++) {
      if (!mounted || _cancelAll) return;
      if (!TtsService.canSpeak(Dars1111.readingLines[i])) continue;
      setState(() => _speakingLine = i);
      await app.tts.speak(Dars1111.readingLines[i]);
    }
    if (!mounted || _cancelAll) return;
    setState(() => _speakingLine = -1);
    await app.quranAudio.playAyah(surah: Dars1111.ayahSurahNum, ayah: Dars1111.ayahNum);
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  if (!TtsService.canSpeak(Dars1111.readingLines[i]))
